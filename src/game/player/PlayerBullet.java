...
//        velocity = new Vector2D(0,0);
        renderer = new Renderer("assets/images/player-bullets/pink/up","over",6);
    ...
    private void checkBarrier() {
        ...
        if(block != null) {
            ...
            if(slideThroughVector == null) {
                ...
                velocity.y = 0;
                this.deactive();
                Explosion explosion=new Explosion(type,this.position.x-25,this.position.y-25);
...
Clip hitSound;
    int count=0;
    public void fire(){
        renderer=new Renderer("assets/images/player-bullets/pink/"+type);
        int range=Settings.long1;
        count++;
        if( count>range*10){
                this.deactive();
                Explosion explosion=new Explosion(type,this.position.x-25,this.position.y-25);
        ...
        Player2 player = GameObject.findIntersects(Player2.class,hitBox,this.index);
